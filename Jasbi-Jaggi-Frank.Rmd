...
<!--mapping problem-->
Word learning is commonly construed as the process of detecting a word form, hypothesizing candidate meanings, and mapping the word to its correct meaning [@clark1993lexicon]. But this is no easy task. For example, an Iranian father holding his child may point to a fish tank and say in Persian: "*mahi*!" After hearing and registering this novel word, the child needs to consider some potential meanings for it. But there are infinitely many concepts that could be the meaning for *mahi*. From "tank", "fish", "smelly", and "look!" to "maybe", "no touching", and "few bubbles", or even complex and absurd concepts like "fish-during-the-day" or "not-yet-disintegrated-animal". As argued by @quine1960word, the meanings of linguistic utterances are always underdetermined by the behavioral data available to the learner. This problem is called many names including "the mapping problem", "the gavagai problem", or "indeterminacy of reference".

There are three general ways of tackling the mapping problem. First, while each learning instance of a word in isolation may be compatible with innumerable candidate meanings, taken together and aggregating across situations a learner may be able to reduce the indeterminacy substantially. For example, the Iranian father may later point to the picture of a fish in a story book and say "*mahi*!" This time there is no "tank" or "bubbles", and the reading may be happing during the night. Therefore, this new observation is incompatible with hypotheses like "tank", "few bubbles", or "fish-during-the-day". If children track which hypotheses fair best across several naming instances, they have a better chance of narrowing down the hypothesis space to the correct answer.

Second, prior linguistic or nonlinguistic knowledge can act as cues to word learning, by either favoring some hypotheses, or partitioning the input in ways that reduce indeterminacy. Considering prior linguistic knowledge, suppose the father says "*mahi ro bebin!*" (look at the fish) instead of just "*mahi*". If the child knows that "*ro bebin*" is an article + verb combination that takes a nominal, then she can limit the hypothesis space to nominal concepts. If in addition she knows that "*bebin*" means "look at", then she can limit her hypotheses to things she can actually look at in the scene. Therefore, prior knowledge of the syntax or semantics of known words, can limit the possible meanings for the unknown word [@brown1957linguistic; ]. Many nonlinguistic aspects of the interaction can also inform word learning and act as cues. For example, the father's pointing or the salience of the fish on the book's page can inform the learner to the meaning of *mahi* as "fish" [@baldwin1993infants; @tomasello2003constructing; @clark2009first].

Third, we can assume that the mechanism of word learning has internal constraints and biases that weed out some candidate hypotheses, a priori. Examples: whole object, taxonomic assumption, mutual exclusivity, etc. 

Learning models can also integrate cues and constraints to develop a learning account that uses multiple cues to home in on the target meaning of a word [@hollich2000breaking].

<!--
First, some theories place constraints or biases internal to the lexicon on either the hypothesis space or the structure of the lexicon. For example, the taxonomic constraint [@markman1984children] proposes that children generate semantic hypotheses for nouns that denote a set of taxonomically related entities and do not hypothesize meanings that capture sets of entities with thematic relations. For example, given the word *gavagai*, children hypothesize the meaning "rabbit" which denotes the set of rabbits (taxonomically related) but not "rabbit and carrot" (thematically related). Therefore, the taxonomic constraint limits the space of hypotheses that the learner entertains. On the other hand, the mutual exclusivity constraint [@markman1988children] as well as the pragmatic principle of contrast [@clark1987principle] limit the structure of the lexicon such that two words are not mapped into the same meaning. This constraint on the lexicon makes the word learning task easier by removing hypotheses that are already associated with learned words.
...
<!--disjunction-->
This study looks at constraints and cues that can aid the acquisition of disjunction. 
Why conjunction and disjunction?
Among functional elements, Quine believed that logical connectives are most likely to be amenable to radical translation. empiricaly grounded, i.e. made concrete similar to concrete content words.
