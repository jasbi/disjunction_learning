...
knitr::opts_chunk$set(fig.crop = F, fig.pos = "H", fig.path='figs/',
...
```{r freqPlots, fig.env="figure", fig.align="center", fig.height=2.5, fig.cap="Left: The relative frequency of \\textit{and/or} (permil) in the speech of parents and children. 95\\% binomial proportion confidence intervals calculated using Agresti-Coull's approximate method. Right: The monthly relative frequency of \\textit{and/or} (permil) in parents and children's speech between 12 and 72 months (1-6 years)."}
by_speaker <-
  freqTable_bySpeaker %>%
  filter(word != "other") %>%
  ...
  labs(x="", y="Relative Frequency (permil)") + 
...
by_age <-
  ...
  labs(x = "Child Age (months)", y="Relative Frequency (permil)") +
...
```{r speechActPlots, fig.align="center", fig.height=2.5, fig.cap="Left: Relative frequency of \\textit{and/or} (permil) in declaratives, imperatives, and interrogatives for parents (green) and children (blue). Right: Proportion of declaratives to questions (percent) in parent-child interactions by child-age."}
...
utteranceType_plot <-
  ...
  labs(x = "Child Age (months)", y="Proportion (percent)") +
...
```{r ageSpeechActPlot, fig.env="figure", fig.align="center", fig.width=5, fig.height=3, fig.cap="Relative frequency of \\textit{and/or} (permil) in declaratives and questions for parents and childern between the child-age of 12 and 72 months (1-6 years)."}
...
answer_prop$answer <- relevel(factor(answer_prop$answer), "No Answer")

disjunctions$appropriate <- "no"
...
answer_plot <-
  ...
  geom_bar(stat = "identity", width=0.7) +
  scale_fill_manual(values=c("gray", "springgreen4", "springgreen3")) +
  labs(x="Child Age (months)", y = "proportion")+
  theme_few() +
  theme(text = element_text(size=10, family="Times"))
